...
void Hooks::Init()
{
	...
	g_Hooks.pvguipanelHook = std::make_unique<VMTHook>(g_VGuiPanel);
	...
	g_Hooks.pvguipanelHook->Hook(vtable_indexes::paintTraverse, Hooks::PaintTraverse);
...
void _stdcall Hooks::PaintTraverse(vgui::VPANEL panel, bool forceRepaint, bool allowForce)
{
	static auto panelId = vgui::VPANEL{ 0 };

	static auto oPaintTraverse = g_Hooks.pvguipanelHook->GetOriginal<PaintTraverse_t>(41);
	...
	if (!panelId) {
		...
		if (g_pEngine->IsInGame() && !g_pEngine->IsTakingScreenshot()) {
